...
- **双数据集验证**:GM(Movie-Game) + AO(Office-Art) 均完成全流程
- LLM+画像方法在 GM 数据集 HR@1 / MRR 上远超 DG 基线(约 157 倍)
- 前端展示系统 4 页面,支持 GM/AO 双数据集切换,全部基于真实项目数据
...
| 测试用户 | 1,000 |
| 平均序列长度 | 20.78 |
| 源域(Office)物品数 | 18,639 |
| 目标域(Art)物品数 | 15,242 |
| 总交互数 | 198,964 |
| 训练指令 | 8,000 条 |
| 测试指令 | 1,000 条 |

> AO 数据集因物品池更大(16,000+ 候选)且物品标题含品牌/型号/尺寸,跨域推荐难度更高。
...
### 路线 C:AO 数据集(Llama2-7B + FP16)

在 GM 数据集基础上,新增 AO(Office-Art)数据集的完整流程。AO 训练使用 FP16 混合精度(非 8bit QLoRA),解决 CUDA 沙箱限制问题。2000 步训练,final_loss=0.3975,eval_loss=0.4266。
...
| 页面 | 功能 |
|------|------|
| 项目概览 | 核心成果卡片、双数据集概览、方法概述 |
| 方法流程 | Pipeline 7 阶段流程、用户画像样例(真实属性)、跨域推荐示意 |
| 推荐展示 | GM/AO 双数据集切换,用户交互历史 + LLM 推荐结果 + 相似用户参考(画像属性 Jaccard 相似度) |
| 效果评测 | GM/AO 双数据集切换,核心指标卡片、训练曲线(训练+验证损失)、DG 基线对比、HR@K & NDCG@K 曲线 |
...
### 结论
1. **双数据集完整流程跑通**:GM(2 epoch / 7894 步)+ AO(2 epoch / 2000 步)均完成训练→推理→评估全链路
2. **LLM+画像方法有效性得到验证**:GM 数据集 HR@1 和 MRR 远超 DG 基线(约 157 倍);AO 数据集 HR@20 提升 4 倍
3. **模型容量是关键**:1.5B(精确匹配 0)→ 7B(精确匹配 1.5%),印证 7B 模型对物品标题记忆更具潜力
4. **前端展示系统完整**,4 页面支持 GM/AO 双数据集切换,全部基于真实数据,评估指标有区分度
