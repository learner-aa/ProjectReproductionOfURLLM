...
### 9.6 AO 数据集(Art-Office)评估结果

**训练配置**:Llama2-7B + LoRA(r=16, α=32),3 epochs / 3000 steps,batch_size=1 + grad_accum=8,FP16
**最终训练 loss**:0.2505 | **最佳 eval_loss**:0.3550
> 轮数选择依据:5 epochs run 在 epoch 4.5 出现 eval_loss 反弹(过拟合),3 epochs 在拐点前停止,train-eval gap 更小(0.10 vs 0.12)。

#### 核心指标

| 指标 | LLM+画像+精炼 | DG 基线 | 说明 |
|------|-------------|---------|------|
| HR@1 | 0.0000 | 0.0000 | AO 物品含品牌+型号+SKU,LLM 无法精确生成 |
| HR@5/10/20 | 0.0000 | 0.0000~0.0003 | 候选池 16000+,精确匹配极难 |
| MRR | 0.0000 | 0.0001 | — |
| fuzzy_HR@1 | 0.0090 | — | 9/1000 样例含关键词重叠 |
| partial_HR@1 | 0.0045 | — | — |
...
| fallback_to_dg | 979/1000 | 97.9% 精炼回退到 DG 模型 |

#### 难度分析
- **候选池规模**:Art 域 15242 物品 + Office 域 18639 物品,远大于 GM 数据集
- **物品标题特征**:含品牌名、型号、SKU 编号(如 "red 73543"、"spr60050"、"fc0032im 2"),LLM 无法记忆精确 ID
- **冷启动占比高**:94.1% 测试用户为冷启动,缺乏交互历史导致推荐困难
- **结论**:AO 数据集本质难度高,3 epochs 已避免过拟合(eval_loss 无反弹),但精确匹配 HR@1 仍为 0,证实此为数据集固有难度而非训练问题,印证原论文中 AO 数据集效果低于 GM 的现象
