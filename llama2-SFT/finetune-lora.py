...
import torch
# 重试 CUDA driver 初始化（沙箱环境下 cudaGetDeviceCount 可能间歇性失败）
import time as _time
_cuda_driver_ok = False
for _attempt in range(10):
    try:
        _t = torch.tensor([1.0]).cuda()
        del _t
        torch.cuda.empty_cache()
        _cuda_driver_ok = True
        print(f"CUDA driver initialized on attempt {_attempt + 1}", flush=True)
        break
    except Exception as _e:
        print(f"CUDA attempt {_attempt + 1} failed: {_e}", flush=True)
        if _attempt < 9:
            _time.sleep(2)
if not _cuda_driver_ok:
    print("WARNING: CUDA driver initialization failed after 10 attempts", flush=True)
import datasets
import evaluate
...
def train():
    print(f"DEBUG [train start]: CUDA={torch.cuda.is_available()}", flush=True)
    # 在参数解析前强制初始化 CUDA driver，创建 CUDA 张量锁定 driver 状态
    _cuda_driver_ok = False
    try:
        _t = torch.tensor([1.0]).cuda()
        _cuda_driver_ok = True
        del _t
        torch.cuda.empty_cache()
        print("DEBUG [cuda tensor]: CUDA driver 初始化成功", flush=True)
    except Exception as e:
        print(f"DEBUG [cuda tensor error]: {e}", flush=True)
    ...
    pass  # send_example_telemetry 已在新版 transformers 移除

    # 如果 NVML 检测失败但 CUDA driver 可用，强制使用 GPU
    if _cuda_driver_ok and str(training_args.device) == "cpu":
        training_args.device = torch.device("cuda:0")
        training_args.n_gpu = 1
        print(f"DEBUG [force GPU]: device={training_args.device}, n_gpu={training_args.n_gpu}", flush=True)
    print(f"DEBUG [after parse_args]: CUDA={torch.cuda.is_available()}, device={training_args.device}", flush=True)
    ...
    if len(tokenizer) > embedding_size:
        model.resize_token_embeddings(len(tokenizer))
    if model_args.load_in_bits in (4, 8):
