...
| 参数 | GM 数据集 | AO 数据集 |
|------|----------|----------|
| LoRA rank (r) | 16 | 8 (对齐原论文) |
| LoRA alpha | 32 | 16 (对齐原论文) |
...
| 训练耗时 | ~11 小时 | ~6.7 小时 |
| 检索增强 | 否 | 是 (retrieval 模板 + 真实 DG 向量, k=1) |
| 早停 | 否 | 是 (patience=3, 未触发) |
...
- lora_r: **8** (本项目 AO 已对齐为 8,GM 为 16)
- lora_alpha: **16** (本项目 AO 已对齐为 16,GM 为 32)
- warmup_steps: 400
- load_in_bits: 8 (8bit QLoRA,本项目 AO 使用 FP16)

**说明**:原论文使用 DeepSpeed 多卡训练,等效 batch=256;本项目单卡训练,等效 batch=8,故减少 epochs 以避免过拟合。AO 数据集已启用 retrieval 模板 + 真实 DG 向量检索增强,对齐原论文的 DG 检索机制。
...
> **改造说明**:本次训练启用了 retrieval 模板 + 真实 DG 向量检索增强 (k=1),对齐原论文的 DG 检索机制。LoRA 参数 r=8/alpha=16 对齐原论文,5 epochs + 早停 (patience=3)。
...
| 训练轮数 | 5 epochs |
| 初始 Loss | 2.4186 |
| 最终 Loss | 0.5316 (train_loss) |
| Best Eval Loss | 0.4450 |
| 训练耗时 | 6.7 小时 |

> **配置说明**:5 epochs + 早停 (patience=3)。eval_loss 全程单调下降 (0.939→0.445),早停未触发,无过拟合。

### 4.2 训练日志关键节点

| Epoch | train_loss | eval_loss | 说明 |
|-------|-----------|-----------|------|
| 0.5 | 0.962 | 0.9393 | 初始收敛 |
| 1.0 | 0.682 | 0.7306 | 快速下降 |
| 1.5 | 0.527 | 0.6344 | 继续下降 |
| 2.0 | 0.467 | 0.5546 | 下降放缓 |
| 2.5 | 0.396 | 0.5224 | 接近收敛 |
| 3.0 | 0.370 | 0.4891 | 持续下降 |
| 3.5 | 0.332 | 0.4730 | 下降趋缓 |
| 4.0 | 0.331 | 0.4610 | 接近最优 |
| 4.5 | 0.288 | 0.4503 | 趋于稳定 |
| 5.0 | 0.296 | 0.4450 | 最终(eval_loss 仍在下降,无反弹) |

> **数据来源**:全部来自当前 5 epochs run 的 `trainer_state.json`(checkpoint-5000)

**无过拟合**:eval_loss 全程单调下降 (0.939→0.445),无反弹;train-eval gap ≈ 0.15。早停未触发,因 eval_loss 持续改善。

### 4.3 评估指标

| 指标 | LLM+检索增强+精炼 | DG 基线 |
|------|------------------|---------|
| HR@1 | **0.0010** | 0.0000 |
| HR@5 | 0.0010 | 0.0000 |
| HR@10 | 0.0010 | 0.0000 |
...
| 指标 | 值 | 说明 |
|------|-----|-----|
| 域外率(原始) | 50.00% (500/1000) | LLM 倾向生成 Office 域泛化文本 |
...
| 指标 | 改造前 (3ep, profile 模板) | 改造后 (5ep, retrieval 模板+真实DG向量) | 变化 |
|------|--------------------------|---------------------------------------|------|
| HR@1 | 0.0000 | **0.0010** | ↑ (从 0 到 1/1000) |
| fuzzy_HR@1 | 0.0090 | 0.0020 | ↓ (下降) |
| partial_HR@1 | 0.0045 | 0.0015 | ↓ (下降) |
| OOD 率 | 49.95% | 50.00% | 持平 |
| eval_loss | 0.3550 | 0.4450 | ↑ (5ep 更高,但 train-eval gap 更大) |
| lora_r / alpha | 16 / 32 | 8 / 16 | 对齐原论文 |

**关键结论**:
- 改造后精确 HR@1 从 0 微升至 0.001 (1/1000),但 fuzzy_HR@1 从 0.009 降至 0.002
- retrieval 模板 + 真实 DG 向量未能显著改善 AO 数据集效果
- 这进一步印证 **AO 的 HR@1 极低不是过拟合或训练不足导致,而是数据集本身的固有难度** (94% 冷启动 + 50% OOD + brand/SKU 精确匹配)
...
4. **50% 域外率**:LLM 倾向生成"合理但错误"的具体物品名
5. **retrieval 增强局限**:k=1 检索的相似用户也面临相同的 brand/SKU 精确匹配困难

**结论**:AO 数据集本质难度高,即使启用 retrieval 增强 + 真实 DG 向量 + 5 epochs 训练,精确匹配 HR@1 仅 0.001,印证原论文中 AO 数据集效果低于 GM 的现象。
...
| 指标 | GM | AO (改造后) | 比值 (GM/AO) |
|------|-----|------------|-------------|
| HR@1 | 0.0275 | 0.0010 | 27.5x |
| HR@20 | 0.0555 | 0.0010 | 55.5x |
| MRR | 0.0345 | 0.0010 | 34.5x |
| OOD 率 | 9.34% | 50.00% | 0.19x |
| 冷启动占比 | 0% | 94.1% | — |
...
| 检索增强 | 不需要 | 已启用 (retrieval+真实DG向量,效果有限) |

---

## 六、AO 改造方案评估结论

### 已执行方案:retrieval 模板 + 真实 DG 向量 (对齐原论文)

| 维度 | 改造内容 | 结果 |
|------|---------|------|
| Prompt 模板 | profile → retrieval (含相似用户 few-shot) | ✓ 已启用 |
| DG 向量 | 近似向量 → 真实 DG 输出向量 | ✓ 已启用 (k=1) |
| LoRA 参数 | r=16/alpha=32 → r=8/alpha=16 | ✓ 对齐原论文 |
| 训练轮数 | 3 epochs → 5 epochs + 早停 | ✓ 已完成 |
| 精确 HR@1 | 0.0000 → 0.0010 | 微升 (1/1000) |
| fuzzy HR@1 | 0.0090 → 0.0020 | 下降 |

**结论**:retrieval 模板 + 真实 DG 向量的改造方案未能显著改善 AO 数据集效果。AO 的低 HR@1 是数据集固有难度导致 (94% 冷启动 + 50% OOD + brand/SKU 精确匹配),非训练方法问题。

### 未执行方案 (备选)

| 方案 | 思路 | 预期 |
|------|------|------|
| ID 生成 | LLM 生成物品 ID 而非标题 | 治本但需重建数据 |
| RAG 检索增强 | DG 预筛 top-100 候选,LLM 选择 | 治本但需改推理流程 |
| 约束解码 | trie/FSM 约束 LLM 只生成真实标题 | 需改解码逻辑 |
| 更大模型 | Llama2-13B 或 Qwen2-72B | 需更多显存 |
