...
| 初始 Loss | 2.3925 |
| 最终 Loss | 0.2505 |
| Best Eval Loss | 0.3550 |
| 训练耗时 | 3 小时 56 分 |

> **配置说明**:3 epochs 是基于 5 epochs run 的 eval_loss 分析确定的最佳轮数。5 epochs 在 epoch 4.5 出现 eval_loss 反弹(过拟合),3 epochs 在拐点前停止。
...
| 1.0 | ~0.8 | 0.5391 | 快速下降 |
| 1.5 | ~0.5 | 0.4486 | 继续下降 |
| 2.0 | ~0.40 | 0.3904 | 下降放缓 |
| 2.5 | ~0.30 | 0.3697 | 接近收敛 |
| 3.0 | 0.2505 | 0.3550 | 最终(eval_loss 仍在下降,无反弹) |

> **数据来源**:全部来自当前 3 epochs run 的 `trainer_state.json`(checkpoint-3000)

**无过拟合**:eval_loss 全程单调下降,无反弹;train-eval gap ≈ 0.10(5 epochs run 为 0.12)
...
| fallback_to_dg | 979/1000 | 97.9% 的精炼回退到 DG 模型 |

### 4.5 3 epochs vs 5 epochs 对比

| 指标 | 3 epochs (当前) | 5 epochs (旧 run) | 变化 |
|------|----------------|-------------------|------|
| HR@1 | 0.0000 | 0.0000 | 持平(均为 0) |
| fuzzy_HR@1 | **0.0090** | 0.0080 | ↑ (微升) |
| partial_HR@1 | **0.0045** | 0.0040 | ↑ (微升) |
| OOD 率 | 49.95% | 49.95% | 持平 |
| eval_loss | 0.3550 | 0.3122 | ↑ (3ep 更高,泛化更好) |
| train-eval gap | 0.10 | 0.12 | ↓ (3ep 更小) |

**关键结论**:3 epochs 减少了过拟合(eval_loss gap 更小),fuzzy_HR@1 和 partial_HR@1 微升,但精确 HR@1 仍为 0。这证明 **AO 的 HR@1=0 不是过拟合导致,而是数据集本身的固有难度**。
