...
### AO 数据集(Office → Art)评估结果

**训练配置**:Llama2-7B + LoRA(r=16, α=32),3 epochs / 3000 steps,FP16 混合精度

| 指标 | LLM+画像+精炼 | DG 基线 |
|------|-------------|---------|
| HR@1 | 0.0000 | 0.0000 |
| HR@5/10/20 | 0.0000 | 0.0000~0.0003 |
| MRR | 0.0000 | 0.0001 |
| fuzzy_HR@1 | 0.0090 | — |
| 域外率(原始) | 49.95% (500/1000) | — |
| 域外率(精炼后) | 50.00% (500/1000) | — |
| 冷启动用户 | 941 / 1000 (94.1%) | — |
| 训练步数 | 3,000 | — |
| 训练轮数 | 3 epochs | — |
| 最终 Loss | 0.2505 | — |
...
**AO 数据集难度说明**:候选池 16000+,物品标题含品牌+型号+SKU 编号(如 "red 73543"、"spr60050"),LLM 无法精确生成 ID;94% 测试用户为冷启动。3 epochs 已避免过拟合(eval_loss 无反弹),但精确匹配 HR@1 仍为 0,证实此为数据集固有难度而非训练问题,印证原论文 AO 数据集效果低于 GM 的现象。
